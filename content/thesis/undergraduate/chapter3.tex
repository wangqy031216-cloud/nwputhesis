...
前两章已经说明，固定奖励函数难以适应策略分布变化，而无约束的动态奖励学习又可能引起奖励尺度膨胀和偏好语义漂移。本章围绕这一矛盾，提出受约束逆强化学习对齐方法PIRO。方法核心是在奖励--策略交替更新过程中引入相邻轮次奖励变化约束，使奖励函数能够随策略分布更新，同时保持局部稳定。

本章先建立奖励学习与策略优化耦合的双层问题，再推导奖励函数诱导出的最优策略及原始目标。在此基础上构造固定旧策略的局部替代目标，证明其在展开点处与原始目标具有相同的一阶信息；给出带误差项的替代目标误差界，刻画有限步更新后替代目标相对于原始目标的偏差；最后将这一约束思想转化为可计算的RMSD反馈机制，并给出完整训练流程。
...
图~\ref{fig:reward_constraint_overview}给出了本章方法的直观动机。无约束奖励更新中，相邻轮次奖励函数会发生较大漂移，由奖励诱导出的策略更新方向也随之摆动，多轮迭代过程不一定稳定接近高质量示范响应分布。受约束奖励更新将相邻奖励函数的变化限制在局部可信范围内，奖励诱导的策略更新轨迹相对平稳。后续各节对这一思想进行形式化：3.1节和3.2节建立奖励诱导策略及外层目标，3.3节证明固定旧策略得到的替代目标在展开点处具有一阶一致性，3.4节给出带误差项的替代目标误差界，3.5节将理论中的奖励变化约束转化为可计算的RMSD反馈机制。

\section{问题建模}

\subsection{符号定义}

设 $\mathcal{X}$ 为提示空间，$\mathcal{Y}$ 为完整响应空间，$\rho$ 为提示分布。本文将大语言模型生成视为序列级单步决策过程：给定提示 $x$ 后，完整回答 $y$ 视为一次动作，在该序列级单步设定下，完整响应的回报由奖励函数 $r_\theta(x,y)$ 直接给出。$\pi_{\mathrm{E}}$ 表示高质量示范响应分布，$\pi_{\mathrm{ref}}$ 表示参考策略，$\pi_\theta$ 表示由奖励函数 $r_\theta$ 诱导出的理论最优策略，$\pi_\phi$ 表示实际训练中的参数化策略，$\beta>0$ 为KL正则系数。
...
\begin{aligned}
    ...
    \mathrm{s.t.}\quad
    \pi_\theta
    &:=\arg\max_\pi\ \mathbb{E}_{x\sim\rho}
    \left[
...
\end{aligned}
\end{equation}
其中，外层目标用于学习奖励函数：通过调整奖励参数 $\theta$，让该奖励函数诱导出的策略 $\pi_\theta$ 倾向于生成高质量示范响应；内层目标在给定奖励函数下优化策略，并通过KL正则限制新策略相对于参考策略的偏离。

该双层形式把大模型偏好对齐中的三个核心对象放在同一框架内：高质量示范响应分布 $\pi_{\mathrm{E}}$ 提供偏好目标，奖励函数 $r_\theta$ 是该偏好目标的可学习表示，诱导策略 $\pi_\theta$ 则给出模型行为。后续推导围绕奖励函数展开，但最终目的始终是改善由奖励诱导出的策略分布，而不是单独提高奖励函数本身的数值。
...
为得到原始双层问题的显式外层目标，本节将内层最优策略的闭式解代入高质量示范响应的对数似然目标，再改写为关于奖励函数及其诱导策略的等价形式。记式~\eqref{eq:pi_ch3}中的归一化项为：
\begin{equation}
    Z_\theta(x)=
...
\begin{equation}
    ...
    \label{eq:log_pi_ch3}
\end{equation}
将式~\eqref{eq:log_pi_ch3}代入外层高质量示范响应似然目标。$\pi_{\mathrm{ref}}$ 与 $\theta$ 无关，对应项可并入常数项，因此有
\begin{equation}
\begin{aligned}
    \beta\ell(\theta)
    &=
...
\begin{equation}
    ...
    \right].
\end{equation}
将上式代入式~\eqref{eq:ell_logz_ch3}后，可得到外层目标中依赖 $\theta$ 的部分。为让高质量示范响应分布项与诱导策略项具有一致的KL正则形式，在不改变关于 $\theta$ 的优化问题的前提下，加入如下与 $\theta$ 无关的高质量示范响应分布KL常数：
\begin{equation}
...
\begin{aligned}
    ...
    \left[r_\theta(x,y)\right]
    +\beta\mathbb{E}_{x\sim\rho}
    \left[D_{\mathrm{KL}}\!\left(\pi_\theta(\cdot|x)\|\pi_{\mathrm{ref}}(\cdot|x)\right)\right].
...
\end{equation}
式~\eqref{eq:l_objective_ch3}是后续PIRO推导的起点。第一项刻画高质量示范响应在当前奖励下的正则化价值，第二项刻画该奖励诱导出的策略自身可达到的软最优值。由于 $\pi_\theta$ 是给定奖励函数下的最优策略，通常有 $J(\pi_\theta,r_\theta)\ge J(\pi_{\mathrm{E}},r_\theta)$，因此 $L(\theta)$ 往往不大于零。最大化 $L(\theta)$ 的含义不是让高质量示范响应项超过最优策略项，而是缩小高质量示范响应与奖励诱导行为之间的软价值差距，使学习到的奖励函数能够更好解释高质量示范响应。由此，偏好对齐问题被具体化为受约束的奖励函数学习问题。
...
\end{equation}
该替代目标的含义是：单轮奖励更新内不显式追踪 $\pi_\theta$ 随 $\theta$ 的变化，转而用固定的旧策略 $\pi_{\mathrm{old}}$ 替代原始目标中的诱导策略项。奖励更新的梯度由高质量示范响应样本与旧策略样本之间的奖励差异估计得到。
...
\end{equation}
式~\eqref{eq:first_order_value_ch3}由 $\pi_{\mathrm{old}}=\pi_{\theta_{\mathrm{old}}}$ 直接得到；梯度一致性需要进一步计算。为保证推导成立，假设响应空间在给定最大生成长度下有限，奖励函数 $r_\theta(x,y)$ 关于 $\theta$ 可微，且 $\pi_{\mathrm{ref}}(y|x)>0$；高质量示范响应分布与旧策略的支持均包含于参考策略支持内。

为计算原始目标的梯度，使用式~\eqref{eq:ell_logz_ch3}中显式依赖 $\theta$ 的形式。式~\eqref{eq:l_from_ell_ch3}中用于对称书写的高质量示范响应分布KL项不依赖 $\theta$，其梯度为零，因此
\begin{equation}
\begin{aligned}
    \nabla_\theta L(\theta)
    &=\nabla_\theta\Bigg\{
...
本节中，我们将证明式~\eqref{eq:lower_bound}。先定义奖励函数的最大逐点变化 $\epsilon$，将误差界证明转化为控制差值 $L_{\mathrm{old}}(\theta_{\mathrm{new}})-L(\theta_{\mathrm{new}})$；再建立两个辅助界，分别控制新旧奖励诱导策略之间的全变差距离以及KL项中的对数策略比；最后将上述差值展开为奖励差异项与KL差异项，分别利用辅助界控制二者，得到式~\eqref{eq:lower_bound}。

\subsection{定理陈述}

\noindent\textbf{定理3.1（替代目标误差界）}
假设：（1）响应空间 $\mathcal{Y}$ 在给定最大生成长度下有限；（2）参考策略满足 $\pi_{\mathrm{ref}}(y|x)>0$，高质量示范响应分布、旧策略与新策略的支持均包含于参考策略支持内；（3）两轮奖励函数有界，即对任意 $(x,y)$ 有 $|r_{\theta_{\mathrm{old}}}(x,y)|\le R$、$|r_{\theta_{\mathrm{new}}}(x,y)|\le R$；（4）相邻奖励函数的最大逐点变化为
\begin{equation}
    \epsilon=
...
\end{gathered}
\end{equation}
该定理说明，替代目标相对于原始目标的高估误差可由奖励函数变化幅度 $\epsilon$ 控制；当 $\epsilon$ 过大时，误差项 $C\epsilon$ 会抵消替代目标带来的改进。需要指出的是，在大语言模型中 $\mathcal{Y}$ 表示完整响应空间，规模极大，因此 $C=3R|\mathcal{Y}|/\beta$ 给出的常数通常较松。该误差界主要用于说明奖励变化幅度与替代目标偏差之间的定性关系，而不是提供可直接用于大模型训练的紧常数。

由式~\eqref{eq:first_order_value_ch3}可知 $L(\theta_{\mathrm{old}})=L_{\mathrm{old}}(\theta_{\mathrm{old}})$。因此，由式~\eqref{eq:lower_bound}可进一步得到
\begin{equation}
    L(\theta_{\mathrm{new}})-L(\theta_{\mathrm{old}})
    \ge
    \left[
    L_{\mathrm{old}}(\theta_{\mathrm{new}})
    -
    L_{\mathrm{old}}(\theta_{\mathrm{old}})
    \right]
    -
    C\epsilon .
    \label{eq:improvement_bound_ch3}
\end{equation}
式~\eqref{eq:improvement_bound_ch3}说明，只有当替代目标的提升量大于奖励变化误差项 $C\epsilon$ 时，原始目标才具有保守改进保证。
...
\end{equation}
为得到该结论，先考察任意响应 $y$ 上的逐点概率差。由式~\eqref{eq:pi_ch3}可知策略具有softmax形式；参考策略项固定时，softmax映射关于奖励扰动满足一个较松的逐点Lipschitz上界：
\begin{equation}
...
将3.3节的一阶等价性与本节的替代目标误差界放在一起，可以得到PIRO从IRL推导走向可训练方法的理论依据：每一轮奖励更新中，固定旧策略构造的替代目标 $L_{\mathrm{old}}$ 与原始目标 $L$ 在当前参数处具有相同的一阶信息，可作为局部优化方向；当奖励函数从 $r_{\theta_{\mathrm{old}}}$ 更新到 $r_{\theta_{\mathrm{new}}}$ 时，诱导策略也会随之改变，替代目标与原始目标之间会产生偏差。式~\eqref{eq:lower_bound}说明，若奖励变化 $\epsilon$ 控制在较小范围内，该偏差可由 $C\epsilon$ 显式上界控制。由此，本文所谓基于逆强化学习的偏好对齐，不只体现在从高质量示范响应反演奖励函数，还体现在对奖励—策略交替更新过程的约束设计上。PIRO的核心原则是：先用旧奖励确定旧策略，再在固定旧策略的条件下更新奖励，同时惩罚相对于旧奖励的变化幅度。
...
L_{\mathrm{old}}(\theta)
-
\lambda\,\mathcal{D}_{r}(\theta,\theta_{\mathrm{old}})
\right].
\end{equation}
其中，左侧表示在旧奖励函数下求得的策略，右侧表示在固定旧策略后优化奖励参数；中间的双向箭头表示交替迭代关系：本轮由 $\theta_{\mathrm{old}}$ 得到 $\pi$，再通过约束奖励更新得到 $\theta_{\mathrm{new}}$，下一轮则将 $\theta_{\mathrm{new}}$ 作为新的旧奖励参数继续迭代。$\mathcal{D}_{r}(\theta,\theta_{\mathrm{old}})$ 表示相邻奖励函数变化的代理度量，理论分析中对应最大逐点差异 $\epsilon$，实际训练中由第3.5节的批次RMSD近似。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/648.PNG}
    \caption{局部替代目标及误差修正后的保守下界示意}
    \label{fig:lower_bound_intuition}
\end{figure}

图~\ref{fig:lower_bound_intuition}给出了上述理论结果的直观解释。蓝色曲线表示原始目标 $L(\theta)$，橙色曲线表示以不同旧参数 $\theta_i$ 为展开点构造的局部替代目标；扣除误差项 $C\epsilon$ 后，才得到相对于原始目标的保守下界。绿色点表示当前展开点，替代目标在该点与原始目标具有相同的一阶信息；紫色点表示沿替代目标更新后的候选参数。每一轮替代目标只在局部可信，若奖励函数更新过大，经误差修正后的保守下界会明显降低，替代目标上的改进难以稳定对应到原始目标。右侧小图表示新旧奖励函数在样本空间 $\mathcal{X}\times\mathcal{Y}$ 上的逐点差异，$\epsilon$ 是这一差异的最大值。控制 $\epsilon$ 的作用是限制每一轮局部替代目标的失真程度，让多轮交替优化更稳定。

据此，本文方法可以概括为围绕上述示意关系展开的交替过程。第 $k$ 轮开始时，先由当前奖励参数得到对应策略快照 $\pi_{\mathrm{old}}$；接着固定该策略快照，用高质量示范响应与旧策略响应的奖励差异构造带奖励变化惩罚的替代目标，奖励更新时限制 $r_\theta$ 相对于 $r_{\theta_{\mathrm{old}}}$ 的变化幅度，避免替代目标过度偏离原始目标；将更新后的奖励参数作为下一轮的旧参数，继续优化策略。上述分析中，3.3节说明代理目标在展开点处具有相同的一阶方向，3.4节说明离开展开点后的代理误差可由奖励变化幅度控制，二者共同支持“受约束奖励更新”的方法原则。

\section{自适应约束调节机制}

\subsection{引入近似约束的必要性}

式~\eqref{eq:lower_bound}表明，替代目标与原始目标之间的偏差由误差项 $C\epsilon$ 控制。在实际训练中不能只最大化 $L_{\mathrm{old}}(\theta)$，还需限制相邻两轮奖励函数的变化幅度。然而，理论形式中的 $C$ 与 $\epsilon$ 都难以直接使用：$C$ 依赖完整响应空间大小 $|\mathcal{Y}|$，$\epsilon$ 需要在所有提示-响应对上取最大值。对于大语言模型，这两个量都无法直接计算；同时，前述常数界较松，更适合作为约束设计的理论动机，而不是直接作为训练超参数。

为此，本文将理论约束转化为可计算的近似形式：用批次级统计量对应 $\epsilon$ 所描述的奖励变化强度，用可调系数对应理论误差项中的常数尺度。该转换不保留严格的全空间下界保证，而是把理论结论转化为训练中的稳定性正则：PIRO并不要求枚举完整响应空间，而是在当前训练分布上观测奖励函数的跨轮变化，并据此调节奖励更新强度。

\subsection{\texorpdfstring{$\epsilon$}{epsilon} 的近似}

为近似理论中的最大逐点变化 $\epsilon$，本文使用相邻两轮奖励函数在同一批次样本上的均方根差。给定批次 $\mathcal{B}=\{(x^{(i)},y^{(i)})\}_{i=1}^{B}$，奖励优化过程中使用的批次RMSD定义为
\begin{equation}
    \mathrm{RMSD}(\theta;\theta_{\mathrm{old}},\mathcal{B})
    =
    \sqrt{
    \frac{1}{B}\sum_{i=1}^{B}
    \left(
    r_\theta(x^{(i)},y^{(i)})
    -
    r_{\theta_{\mathrm{old}}}(x^{(i)},y^{(i)})
    \right)^2
    +\zeta
    }.
    \label{eq:rmsd_function}
\end{equation}
其中 $\zeta>0$ 为数值稳定项，实际取值很小，用于避免平方根在零点附近产生不稳定梯度。该量是当前奖励参数 $\theta$ 的函数，可作为奖励目标中的惩罚项参与反向传播。每轮奖励优化完成后，再用更新后的奖励参数计算轮次级统计量
\begin{equation}
    \mathrm{RMSD}_k
    =
    \mathrm{RMSD}(\theta_k;\theta_{k-1},\mathcal{B}_k),
    \label{eq:rmsd}
\end{equation}
其中 $\theta_k$ 和 $\theta_{k-1}$ 分别为第 $k$ 轮与第 $k-1$ 轮的奖励参数，$\mathcal{B}_k$ 为本轮用于统计奖励变化的样本批次。RMSD不是全空间最大值 $\epsilon$ 的严格上界，而是训练批次上的可计算代理量，用于反映当前训练分布上的奖励变化强度。实现中也可使用 $\mathrm{RMSD}^2$ 或均方误差形式作为更平滑的惩罚项；本文采用RMSD，是为了让该统计量与奖励输出差异保持相同量纲。

\subsection{\texorpdfstring{$C$}{C} 的自适应近似}

理论常数 $C=3R|\mathcal{Y}|/\beta$ 无法在完整响应空间上直接计算。为在实现中保留“惩罚过大奖励变化”的作用，本文引入可调系数 $\eta_k$，并根据RMSD反馈自适应更新 $\eta_k$。设 $\tau>0$ 为目标RMSD阈值，$\gamma\in(0,1)$ 为下阈值系数，$\delta>1$ 为上阈值系数，$\alpha\in(0,1)$ 为衰减因子，$\alpha'>1$ 为增长因子，$\eta_{\min}$ 与 $\eta_{\max}$ 分别为约束系数下界和上界，调节规则为：
...
\begin{equation}
    ...
    \mathrm{clip}\left(
    \begin{cases}
        \alpha' \cdot \eta_k, & \text{若 } \mathrm{RMSD}_k > \delta\tau, \\
        \alpha \cdot \eta_k, & \text{若 } \mathrm{RMSD}_k < \gamma\tau, \\
        \eta_k, & \text{其他},
    \end{cases}
    \eta_{\min},\eta_{\max}
    \right).
...
\end{equation}
当RMSD大于上阈值 $\delta\tau$ 时，增大 $\eta_k$ 以加强对奖励变化的惩罚；当RMSD小于下阈值 $\gamma\tau$ 时，减小 $\eta_k$ 以放松约束；$\eta_{\min}$ 和 $\eta_{\max}$ 分别用于避免约束项消失或过强。因此，$\eta_k\cdot\mathrm{RMSD}(\theta;\theta_{\mathrm{old}},\mathcal{B})$ 并不构成对理论误差项 $C\epsilon$ 的严格替代，而是在当前训练分布上对奖励变化强度的可计算近似。本文将其作为受理论启发的稳定性正则项，用于在实际训练中抑制相邻轮次奖励函数的过大漂移。

\section{完整训练流程}

\subsection{算法总体设计}

基于上述理论框架，本文提出近端逆奖励优化方法（Proximal Inverse Reward Optimization, PIRO）。其中，“逆奖励学习”表示从高质量示范响应中学习可用于策略优化的奖励函数，“近端”表示每轮奖励更新受到相邻轮次奖励变化幅度的约束。PIRO采用奖励与策略交替优化的迭代训练范式，总体流程如算法~\ref{alg:piro}所示。
...
\begin{algorithm}[H]
    ...
    \small
    \begin{algorithmic}[1]
        \Require 高质量示范响应数据集 $\mathcal{D}_E$，提示数据集 $\mathcal{D}$，参考策略 $\pi_{\mathrm{ref}}$，
                 初始奖励参数 $\theta_0$，初始策略参数 $\phi_0$，
                 迭代轮数 $K$，每轮内层步数 $T_\pi$，每轮外层步数 $T_r$，
                 奖励学习率 $\lambda^r$，策略学习率 $\lambda^\pi$，初始约束系数 $\eta_0$，
                 RMSD目标阈值 $\tau$，下阈值系数 $\gamma$，上阈值系数 $\delta$，衰减因子 $\alpha$，增长因子 $\alpha'$，
                 约束系数范围 $[\eta_{\min},\eta_{\max}]$
        \Ensure 对齐后的策略参数 $\phi$
        \State $\theta \gets \theta_0,\ \phi \gets \phi_0,\ \eta \gets \eta_0$
        \For{$k = 1, 2, \ldots, K$}
            \State \textbf{// 内层：策略优化}
            \For{$t = 1, 2, \ldots, T_\pi$}
                ...
                \State 用PPO/GRPO等KL约束策略优化器近似最大化 $\mathcal{J}_\pi$，更新 $\phi$
            \EndFor
            \State \textbf{// 外层：奖励优化（含自适应约束）}
            \State 固定当前奖励参数与策略快照：$\theta_{\mathrm{old}} \gets \theta,\ \pi_{\mathrm{old}}\gets\pi_\phi$
            \For{$t = 1, 2, \ldots, T_r$}
                \State 从 $\mathcal{D}_E$ 采样高质量示范响应批次 $\{(x^{(i)}, y_E^{(i)})\}$
                \State 从 $\mathcal{D}$ 采样提示，并用固定策略 $\pi_{\mathrm{old}}$ 采样响应批次 $\{(x^{(j)}, y_{\mathrm{old}}^{(j)})\}$
                \State 取奖励约束批次 $\mathcal{B}_r$，计算 $\mathcal{R}_{\mathrm{prox}}=\mathrm{RMSD}(\theta;\theta_{\mathrm{old}},\mathcal{B}_r)$
                \State 计算带批次RMSD惩罚的奖励目标 $L_{\mathrm{old}}(\theta)-\eta\cdot\mathcal{R}_{\mathrm{prox}}$
                \State $g_\theta \gets \nabla_\theta\left(\mathbb{E}[r_\theta(x,y_E)]-\mathbb{E}[r_\theta(x,y_{\mathrm{old}})]-\eta\cdot\mathcal{R}_{\mathrm{prox}}\right)$
                \State 执行梯度上升：$\theta \gets \theta + \lambda^r g_\theta$
            \EndFor
            \State \textbf{// 自适应约束系数调节}
            \State 在本轮奖励优化使用的样本批次上计算 $\mathrm{RMSD}_k$（见式~\eqref{eq:rmsd}）
            \If{$\mathrm{RMSD}_k > \delta\tau$}
                \State $\eta \gets \mathrm{clip}(\alpha' \cdot \eta,\eta_{\min},\eta_{\max})$
            \ElsIf{$\mathrm{RMSD}_k < \gamma\tau$}
                \State $\eta \gets \mathrm{clip}(\alpha \cdot \eta,\eta_{\min},\eta_{\max})$
            \Else
                \State $\eta \gets \mathrm{clip}(\eta,\eta_{\min},\eta_{\max})$
            \EndIf
        \EndFor
        \State \Return $\phi$
    \end{algorithmic}
...
\textbf{内层策略优化}：固定当前奖励参数 $\theta$，以 $T_\pi$ 步更新策略参数 $\phi$，让策略趋向当前奖励函数下的近似最优解。优化目标为期望奖励与KL惩罚的加权和，KL惩罚在词元级别计算，即 $\log(\pi_\phi(y|x)/\pi_{\mathrm{ref}}(y|x)) = \sum_t \log(\pi_\phi(a_t|s_t)/\pi_{\mathrm{ref}}(a_t|s_t))$。实际实现中，内层策略优化可由PPO、GRPO或其他KL约束策略优化器近似完成；算法中用 $\mathcal{J}_\pi$ 表示其抽象更新目标。

\textbf{外层奖励优化}：固定当前策略为 $\pi_{\mathrm{old}}$，以 $T_r$ 步梯度上升更新奖励参数 $\theta$，最大化带批次RMSD惩罚的替代目标 $L_{\mathrm{old}}(\theta)-\eta\cdot\mathrm{RMSD}(\theta;\theta_{\mathrm{old}},\mathcal{B})$。本步固定 $\pi_{\mathrm{old}}$，KL项对 $\theta$ 不产生梯度，实现时用高质量示范响应样本奖励减去旧策略样本奖励，并加入RMSD惩罚项来估计奖励更新方向。这里的 $\mathrm{RMSD}(\theta;\theta_{\mathrm{old}},\mathcal{B})$ 随当前奖励参数 $\theta$ 变化，因此参与奖励更新的反向传播。

\textbf{自适应约束系数调节}：每轮奖励优化完成后，计算本轮奖励函数相对于上轮的轮次级统计量 $\mathrm{RMSD}_k$，依据式~\eqref{eq:adaptive_eta}调节下一轮的约束系数。当RMSD超过 $\delta\tau$ 时增大 $\eta$，当RMSD低于 $\gamma\tau$ 时减小 $\eta$，并通过 $[\eta_{\min},\eta_{\max}]$ 裁剪避免约束项消失或过强。因此，训练中的RMSD惩罚项用于当前轮反向传播，轮次结束后的 $\mathrm{RMSD}_k$ 用于调节下一轮约束强度。

\section{本章小结}

本章围绕受约束逆强化学习的大模型对齐方法展开，主要内容包括：（1）将对齐问题写成高质量示范响应似然最大化的双层目标，外层学习奖励参数，内层求解KL正则最优策略；（2）推导内层问题的Boltzmann闭式策略，说明奖励通过指数加权影响参考策略；（3）构造固定旧策略的替代目标，并说明其与原始目标在展开点处一阶一致；（4）给出替代目标误差界与保守改进条件，说明原始目标不低于替代目标减去误差项 $C\epsilon$，奖励更新幅度需要受控；（5）在实际实现中用批次RMSD监控奖励变化，并据此自适应调整约束系数。至此，基于逆强化学习的偏好奖励反演被转化为PIRO的可执行训练流程：奖励函数可以随策略分布更新，但其跨轮变化必须受到近端约束。第四章将通过实验检验该约束机制对对齐效果和训练稳定性的影响。

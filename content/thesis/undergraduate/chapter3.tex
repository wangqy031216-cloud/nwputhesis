...
前两章已经说明，固定奖励函数难以适应策略分布变化，而无约束的动态奖励学习又可能引起奖励尺度膨胀和语义漂移。本章围绕这一矛盾，提出受约束逆强化学习对齐方法PIRO。方法核心是在奖励--策略交替更新过程中引入相邻轮次奖励变化约束，使奖励函数能够随策略分布更新，同时保持局部稳定。

本章先建立奖励学习与策略优化耦合的双层问题，再推导奖励函数诱导出的最优策略及原始目标。在此基础上构造固定旧策略的局部替代目标，证明其在展开点处与原始目标具有相同的一阶信息；给出带误差项的改进下界，刻画有限步更新后替代目标相对于原始目标的偏差；最后将这一约束思想转化为可计算的RMSD反馈机制，并给出完整训练流程。
...
设 $\mathcal{X}$ 为提示空间，$\mathcal{Y}$ 为完整响应空间，$\rho$ 为提示分布。本文将大语言模型生成视为序列级单步决策过程：给定提示 $x$ 后，完整回答 $y$ 视为一次动作，因此动作价值函数 $Q(x,y)$ 可直接写成奖励 $r_\theta(x,y)$。$\pi_{\mathrm{E}}$ 表示专家响应分布，$\pi_{\mathrm{ref}}$ 表示参考策略，$\pi_\theta$ 表示由奖励函数 $r_\theta$ 诱导出的理论最优策略，$\pi_\phi$ 表示实际训练中的参数化策略，$\beta>0$ 为KL正则系数。
...
该双层形式把大模型偏好对齐中的三个核心对象放在同一框架内：专家分布 $\pi_{\mathrm{E}}$ 提供偏好目标，奖励函数 $r_\theta$ 是该偏好目标的可学习表示，诱导策略 $\pi_\theta$ 则给出模型行为。后续推导围绕奖励函数展开，但最终目的始终是改善由奖励诱导出的策略分布，而不是单独提高奖励函数本身的数值。
...
\end{equation}
式~\eqref{eq:l_objective_ch3}是后续PIRO推导的起点。第一项刻画专家偏好响应在当前奖励下的正则化价值，第二项刻画该奖励诱导出的策略自身可达到的软最优值。由于 $\pi_\theta$ 是给定奖励函数下的最优策略，通常有 $J(\pi_\theta,r_\theta)\ge J(\pi_{\mathrm{E}},r_\theta)$，因此 $L(\theta)$ 往往不大于零。最大化 $L(\theta)$ 的含义不是让专家项超过最优策略项，而是缩小专家行为与奖励诱导行为之间的软价值差距，使学习到的奖励函数能够更好解释专家响应。由此，偏好对齐问题被具体化为受约束的奖励函数学习问题。
...
假设：（1）响应空间 $\mathcal{Y}$ 在给定最大生成长度下有限；（2）参考策略满足 $\pi_{\mathrm{ref}}(y|x)>0$，专家分布、旧策略与新策略的支持均包含于参考策略支持内；（3）两轮奖励函数有界，即对任意 $(x,y)$ 有 $|r_{\theta_{\mathrm{old}}}(x,y)|\le R$、$|r_{\theta_{\mathrm{new}}}(x,y)|\le R$；（4）相邻奖励函数的最大逐点变化为
\begin{equation}
    \epsilon=
...
\end{gathered}
\end{equation}
该定理说明，替代目标相对于原始目标的高估误差可由奖励函数变化幅度 $\epsilon$ 控制；当 $\epsilon$ 过大时，误差项 $C\epsilon$ 会抵消替代目标带来的改进。需要指出的是，在大语言模型中 $\mathcal{Y}$ 表示完整响应空间，规模极大，因此 $C=3R|\mathcal{Y}|/\beta$ 给出的常数通常较松。该下界主要用于说明奖励变化幅度与替代目标偏差之间的定性关系，而不是提供可直接用于大模型训练的紧常数。
...
\end{equation}
该量是当前奖励参数 $\theta$ 的函数，可作为奖励目标中的惩罚项参与反向传播。每轮奖励优化完成后，再用更新后的奖励参数计算轮次级统计量
\begin{equation}
    \mathrm{RMSD}_k
    =
...
理论常数 $C=3R|\mathcal{Y}|/\beta$ 无法在完整响应空间上直接计算。为在实现中保留“惩罚过大奖励变化”的作用，本文引入可调系数 $\eta_k$，并根据RMSD反馈自适应更新 $\eta_k$。设 $\tau>0$ 为目标RMSD阈值，$\gamma\in(0,1)$ 为下阈值系数，$\delta>1$ 为上阈值系数，$\alpha\in(0,1)$ 为衰减因子，$\alpha'>1$ 为增长因子，调节规则为：
...
\end{equation}
当RMSD大于上阈值 $\delta\tau$ 时，增大 $\eta_k$ 以加强对奖励变化的惩罚；当RMSD小于下阈值 $\gamma\tau$ 时，减小 $\eta_k$ 以放松约束。因此，$\eta_k\cdot\mathrm{RMSD}(\theta;\theta_{\mathrm{old}},\mathcal{B})$ 并不构成对理论误差项 $C\epsilon$ 的严格替代，而是在当前训练分布上对奖励变化强度的可计算近似。本文将其作为受理论启发的稳定性正则项，用于在实际训练中抑制相邻轮次奖励函数的过大漂移。
...
\textbf{外层奖励优化}：固定当前策略为 $\pi_{\mathrm{old}}$，以 $T_r$ 步梯度上升更新奖励参数 $\theta$，最大化带批次RMSD惩罚的替代目标 $L_{\mathrm{old}}(\theta)-\eta\cdot\mathrm{RMSD}(\theta;\theta_{\mathrm{old}},\mathcal{B})$。本步固定 $\pi_{\mathrm{old}}$，KL项对 $\theta$ 不产生梯度，实现时用专家样本奖励减去旧策略样本奖励，并加入RMSD惩罚项来估计奖励更新方向。这里的 $\mathrm{RMSD}(\theta;\theta_{\mathrm{old}},\mathcal{B})$ 随当前奖励参数 $\theta$ 变化，因此参与奖励更新的反向传播。

\textbf{自适应约束系数调节}：每轮奖励优化完成后，计算本轮奖励函数相对于上轮的轮次级统计量 $\mathrm{RMSD}_k$，依据式~\eqref{eq:adaptive_eta}调节下一轮的约束系数。当RMSD超过 $\delta\tau$ 时增大 $\eta$，当RMSD低于 $\gamma\tau$ 时减小 $\eta$。因此，训练中的RMSD惩罚项用于当前轮反向传播，轮次结束后的 $\mathrm{RMSD}_k$ 用于调节下一轮约束强度。
